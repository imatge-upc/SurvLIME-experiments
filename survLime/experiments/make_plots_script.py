...
def generate_plots_real_datasets():
    results_folder = os.path.join(os.path.dirname(os.getcwd()), "computed_weights_csv", "exp_real_datasets")

    models = ["cox", "rsf","xgb"]#"deepsurv","deephit"]
    datasets = ["lung", "veterans", "udca"] # "pbc"
    for dataset in datasets:
        for model_type in models:
            data = pd.read_csv(results_folder+f"/{model_type}_exp_{dataset}_surv_weights.csv")
            ...
            print(f"Figure saved in figures/{model_type}_{dataset}.png")
...
def generate_plots_simulated_experiments():
    for i in [1, 2]:
        for group in ["mean","min","max"]:
            ...
            plt.savefig(f"figures/simulated_exp_cluster{i}_{group}_values.png",  bbox_inches="tight", dpi=200)
            print(f"Figure saved in figures/simulated_exp_cluster{i}_{group}_values.png")
...
def generate_deepmodels_rds_plots():
    ...
    """
    results_folder = os.path.join(os.path.dirname(os.getcwd()), "computed_weights_csv", "exp_deepsurv_rds")
    data = pd.read_csv(results_folder+f"/exp_deepsurv_rds_surv_weights.csv")
    
    p = create_boxenplot(data)

    p.set_title(f"DeepSurv RSD SurvLIME values", fontsize= 16, fontweight="bold");

    plt.savefig(f"figures/deepsurv_rds.png",  bbox_inches="tight", dpi=200)
    print("Figure saved in figures/deepsurv_rds.png")
...
def plot_experiment_1():
    ...
    for exp, coefficients in zip([1,2], [coefficients_1, coefficients_2]):
        ...
        # Save the figure
        plt.savefig(f"figures/exp_1.{exp}_results.png", bbox_inches = "tight", dpi=200)   
