    ...
    for exp, coefficients in zip([1, 2], [coefficients_1, coefficients_2]):
        for i in tqdm(range(50)):
            ...
            dist = euclidean_distances(
                compt_weights, np.array(coefficients).reshape(1, -1)
            )
            ...
            if i != 0:
                if (
                    compt_weights.iloc[furthest].sort_values(ascending=False).index
                    == max_dfs[-1].sort_values(ascending=False).index
                ).all():
                    pass
                else:
                    print(i)
        ...
        dfs_together_max = dfs_together_max.reindex(
            dfs_together_max.median().sort_values(ascending=False).index, axis=1
        )
        dfs_together_min = dfs_together_min.reindex(
            dfs_together_min.median().sort_values(ascending=False).index, axis=1
        )
        dfs_together_mean = dfs_together_mean.reindex(
            dfs_together_mean.median().sort_values(ascending=False).index, axis=1
        )
        ...
        sns.boxenplot(
            x="variable",
            y="value",
            data=pd.melt(dfs_together_mean),
            ax=ax[0],
            palette="RdBu",
        )
        ax[0].set_title(f"Set {exp} mean values")
        ...
        sns.boxenplot(
            x="variable",
            y="value",
            data=pd.melt(dfs_together_min),
            ax=ax[1],
            palette="RdBu",
        )
        ax[1].set_title(f"Set {exp} best values")
        ...
        sns.boxenplot(
            x="variable",
            y="value",
            data=pd.melt(dfs_together_max),
            ax=ax[2],
            palette="RdBu",
        )
        ax[2].set_title(f"Set {exp} worst values")
...
def compare_cox_weights_with_survlimepy():
    ...
    for dataset in datasets:
        ...
        if os.path.exists(file_path):
            ...
            raise ValueError(
                f"Experiments with real datasets have not been run yet. Please run the experiments first."
            )

        print(f"Dataset: {dataset}")
        for name, cox, survlime_weight in zip(
            model.feature_names_in_, cox_weights, data.median().values
        ):
            print(
    ...
    print("Random Survival Dataset and DeepSurv model")
